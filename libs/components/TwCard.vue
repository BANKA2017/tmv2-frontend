<template>
  <div id="twCard" @click="(e) => {e.stopPropagation()}">
    <div :class="{card: object.secondly_type !== 'image_collection_website', 'mb-3': true, 'background-second': !(object.type === 'unified_card' && object.secondly_type === 'image_collection_website'), }" style="border-radius: 14px 14px 14px 14px">
      <a v-if="object?.url && (object.secondly_type === 'media_with_details_horizontal' || object.secondly_type === 'twitter_article' || object.type !== 'unified_card') && object.type !== 'audiospace' && object.type !== 'broadcast' && object.type !== 'periscope_broadcast' && object.type !== 'live_event'" :href="object.url" class="stretched-link text-decoration-none" target="_blank"></a>
      ...
      <tw-broadcast v-else-if="object.type === 'broadcast' || object.type === 'periscope_broadcast' || object.type === 'live_event'" :tweet_id="object.tweet_id" :cover="predictiveMedia.cover" :url="object.url" :title="object.title"/>
